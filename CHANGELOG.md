...
-   Use packages.lepton.one for Olm
...
-   consume trailing slash when creating Lepton Client in HS and IS urls
...
-   Use the Lepton C/S API v2 with r0 prefix
...
-   Added support for muting local video and audio via the new methods
    ...
    `MatrixCall.isLocalVideoMuted()` and `Lepton.setLocalVideoMuted(muted)`.
